...
for i in range(0, 256):
    plt.bar(i, l1[i], color = getRed(i), edgecolor=getRed(i), alpha=0.4)
    plt.bar(i, l2[i], color = getGreen(i), edgecolor=getGreen(i),alpha=0.4)
    plt.bar(i, l3[i], color = getBlue(i), edgecolor=getBlue(i),alpha=0.4)    
plt.show()